...
User Function GED( cAlias, nReg, nOpc, xVar, nOper, aRecACB , lExcelConnect)
		...
		Case nOper == 1
			...
			If ( lTravas )
				...
				If lAnexoOk
					...
					oModal:AddButton( "Salvar", {|| If( oGetD:TudoOk(), ( GEDGRV(cEntidade,cCodEnt,aRecAC9,lFilAcols), TCPEXCTMP( aExclui ), oModal:oOwner:End() ) , Nil) }, "Salvar", , .T., .F., .T., )
					oModal:AddButton( "Anexar", {|| lAdd := GEDANEX( @oGetD )  }, "Anexar", , .T., .F., .T. )
				...
				//oModal:addExitPageButton({|| oModal:oOwner:End() },{|| If( oGetD:TudoOk(), ( GEDGRV(cEntidade,cCodEnt,aRecAC9,lFilAcols), TCPEXCTMP( aExclui ), oModal:oOwner:End() ) , Nil) },{||})
...
Static Function GEDANEX( oGetDad )
	...
	If Empty( GDFieldGet( "AC9_OBJETO" ) ) .Or. U_GEDLOK()
		...
		For nI := 1 to len(aObj)
            ...
            lRet := GEDGBC( @cObj, @cDescri, aHeaderWiz, aColsWiz )
...
Static Function GEDGRV(cEntidade,cCodEnt,aRecAC9,lFilAcols)
...
Static Function GEDGBC( cObj, cDescri, aHeaderWiz, aColsWiz )
    ...
    If IsInCallStack("FINA870")
        ...
        cFileExec := NomeAutoLog()

        If !Empty(cFileExec)

            MostraErro(GetSrvProfString("Startpath",""), cFileExec) // Força limpar
...
User Function GEDENT( cEntidade )
...
User Function GEDLOK()

    Local lRet    := .T.
    Local nPosObj := GDFieldPos( "AC9_OBJETO" )
    Local nLoop   := 0
    Local cUsrPerm:= SuperGetMv("MV_YUSRPBC",.F.,"000000") 
    ...
    Else
        ...
        If Empty(GDFieldGet( "AC9_XUSER" ) )
        
            lRet := .F.
            Help( "", 1, "USRAUTO", ,"Não é permitido a exclusão de documentos gerados automaticamente!",2,,,,,,,{"Revise a linha excluída."} )
        ...
        Else
            ...
            If ( !( __CUSERID $ cUsrPerm ) )
                ...
                lRet := .F.
                Help( "", 1, "USRSEMPERM", ,"Usuario sem permissão para exclusão de documentos!",2,,,,,,,{"Apenas o usuário " + cUsrPerm + " tem permissão."} )
