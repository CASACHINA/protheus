...
User Function DocGrvGED(cAnexo, cFileName)
	...
	If File(cDirDoc + "\" + cAnexoGrv)

	// 	TLogConsole():Log("O arquivo [" + cDirDoc + "\" + cAnexoGrv + "] foi copiado para o server.", "TCCO04KM")

	// Else

	// 	TLogConsole():Log("O arquivo [" + cDirDoc + "\" + cAnexoGrv + "] não foi copiado para o server.", "TCCO04KM", 3)
	...
	AC9->AC9_XDATA  := Date()
	AC9->AC9_CODENT	:= U_GEDENT( cEntidade )[1]
...
Static function getEntidade()
	...
	If cAlias == 'SF1'
		...
		// Comentado pois ja estara posicionado, da forma que estava sempre posicionava no primero item do SD1!
