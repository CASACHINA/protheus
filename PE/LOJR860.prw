...
If lContinua
	SetPrvt('oPrint')
	oPrint:= TMSPrinter():New("Nota de Crédito")
	oPrint:SetPortrait()
	...
	MsgRun("Gerando Visualização, Aguarde...","",{|| CursorWait(), Lj850Print(oPrint,aDocDev,aRecSD2,aDadosNCC),CursorArrow()})
	...
	While SE1->(!Eof()) .AND. xFilial("SE1")		== SE1->E1_FILIAL 	.AND. ;
	   	...
	   	If !( SE1->E1_SITUACA $ "27" .OR. SE1->E1_SALDO == 0 )
     		...
     		If ( SE1->E1_TIPO $ MV_CRNEG )
				...
				oPrint:Say( 0180, 0875, "Nota de Crédito", oFont26N )
				...
				*/
				...
				If SA1->(DbSeek(xFilial("SA1")+aDocDev[3]+aDocDev[4] ))
					oPrint:Say( 400, 0200, "Código:", oFontCN12N ) //
				...
				oPrint:Say( 480, 0200, "Data de Devolução:", oFontCN12N ) //
				...
				oPrint:Say( 560, 0750, "Número:", oFontCN12N ) //
				...
				oPrint:Say( 800, 0200, "Código", oFontCN12N ) //
				oPrint:Say( 800, 0750, "Descrição", oFontCN12N ) //
