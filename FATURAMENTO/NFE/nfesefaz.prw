	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			ElseIf FunName() == "SPEDNFSE"
				...
				// Personalizacao - B2B - Casa China
				If "/" $ FisGetEnd(SA1->A1_END,SA1->A1_EST)[3]
					aadd(aDest,IIF(FisGetEnd(SA1->A1_END,SA1->A1_EST)[3]<>"",FisGetEnd(SA1->A1_END,SA1->A1_EST)[3], IF(!empty(SA1->A1_YNUMERO), SA1->A1_YNUMERO, "SN")))
				Else
	 				aadd(aDest,IIF(FisGetEnd(SA1->A1_END,SA1->A1_EST)[2]<>0,FisGetEnd(SA1->A1_END,SA1->A1_EST)[2], IF(!empty(SA1->A1_YNUMERO), SA1->A1_YNUMERO, "SN")))
				EndIf
			...
			If SF1->F1_TIPO $ "DB" 
				...
				If SF1->(FieldPos("F1_FORRET"))<>0 .And. !Empty(SF1->F1_FORRET+SF1->F1_LOJARET) .And. SF1->F1_FORRET+SF1->F1_LOJARET <> SF1->F1_FORNECE+SF1->F1_LOJA
					...
					IF MsSeek(xFilial("SA1")+SF1->F1_FORRET+SF1->F1_LOJARET)
						...
						aadd(aRetirada,SA1->A1_CGC)
						aadd(aRetirada,MyGetEnd(SA1->A1_END,"SA1")[1])
						aadd(aRetirada,ConvType(IIF(MyGetEnd(SA1->A1_END,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,"SA1")[2],IF(!empty(SA1->A1_YNUMERO), SA1->A1_YNUMERO, "SN"))))
				...
				If SF1->(FieldPos("F1_FORENT")) <> 0 .And. !Empty(SF1->F1_FORENT+SF1->F1_LOJAENT) .And. SF1->F1_FORENT+SF1->F1_LOJAENT <> SF1->F1_FORNECE+SF1->F1_LOJA
					...
					If MsSeek(xFilial("SA1")+SF1->F1_FORENT+SF1->F1_LOJAENT)
						...
						aadd(aEntrega,MyGetEnd(SA1->A1_END,"SA1")[1])
						//ajuste Casa China
						aadd(aEntrega,ConvType(IIF(MyGetEnd(SA1->A1_END,"SA1")[2]<>0,MyGetEnd(SA1->A1_END,"SA1")[2],IF(!empty(SA1->A1_YNUMERO), SA1->A1_YNUMERO, "SN"))))
				...
				*/
				...
				If cDevMerc == "S"
					...
					If !lEndFis
						...
						Else
						// Personalizacao Casa China
							if !empty(SA1->A1_YNUMERO)
								aadd(aDest,SA1->A1_YNUMERO)
							else
								aadd(aDest,"SN")
							endif
						EndIf
